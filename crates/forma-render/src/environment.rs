//! Shared bounded HDR decoding and procedural lighting, independent of GPU API.
...
pub(crate) const ENV_WIDTH: u32 = 512;
pub(crate) const ENV_HEIGHT: u32 = 256;
pub(crate) const SPECULAR_WIDTH: u32 = 256;
pub(crate) const SPECULAR_HEIGHT: u32 = 128;
pub(crate) const SPECULAR_SLICES: u32 = 9;
pub(crate) const CACHE_CAPACITY: usize = 4;

pub(crate) fn load_environment(key: &EnvironmentKey) -> Result<Vec<[f32; 4]>> {
    match key {
        EnvironmentKey::Builtin(studio) => Ok(builtin(*studio)),
        EnvironmentKey::File { path, .. } => Ok(resize_radiance(&decode_hdr(path)?)),
    }
}
