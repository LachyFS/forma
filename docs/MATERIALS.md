...
- **Custom** opens a native multiline Metal editor. **Compile & apply** (⌘Return)
  ...
  ⌘Z inside the editor edits the draft's separate undo history.
...
Enter a **Metal function body**, not a complete kernel. Texture slots are sampled
...
For example, keep the selected image texture and add stripes:
...
when active source changes; camera, texture and scalar edits reuse them. Applying
...
the film. Source is native GPU code, not a sandboxed language; successful compilation
...
Runtime compilation uses Metal's
...
On a Mac, `cargo test --locked -p forma-render` also runs actual Metal texture,
glass, custom compilation, fallback and recovery regressions. Runtime compilation
and native GPU output must be verified on macOS; cross-target Rust checks do not
